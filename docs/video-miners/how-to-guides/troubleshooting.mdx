...
[session limit guide](./limits.md) for information on setting the session limit.
...
Transcoders will not be able to serve the Livepeer network if they are behind a
NAT (eg, a home router). If this is the case, special accommodations must be
made for the transcoder, such as port forwarding or putting the the transcoder
in the DMZ. The only port that is required to be public is the one that was set
during the transcoder registration step (default 8935). Be aware that there are
many risks to running a public server. Only set up a transcoder if you are
comfortable with managing these risks.
...
service URI by storing it on the blockchain. During reigstration you are only
...
<Pagination
  previousStep={{
    title: "Dual Mining",
    href: "/docs/video-developers/how-to-guides/dual-mining",
  }}
/>
