...
- Make sure `livepeer_bench` is installed by following the
  [installation guide](https://github.com/livepeer/go-livepeer/blob/master/doc/install.md).
  `livepeer_bench` is included in all binary releases and can also be built
  using the build from source instructions
...
When all the transcoding sessions end it will output the total time taken for
transcoding, total number of segments in the stream, and the total duration of
the segments:
...
`-concurrentSessions` and compare the total time taken.
