...
If you are using CUDA MPS, start the MPS server by following
...
1. Wait until the miner completes DAG generation

2. Run `livepeer` on the same machine as the miner and using the same GPU device
   IDs as the miner with the `-nvidia` flag

3. `livepeer` will need to be stopped when the miner re-generates the DAG to
   avoid transcoding issues and can be re-started when DAG generation completes
...
<Pagination
  ...
  nextStep={{
    title: "Set Pricing",
    href: "/docs/video-miners/how-to-guides/pricing",
  }}
/>
