...
livepeer \
    -network mainnet \
    -ethUrl <ETH_URL> \ # API URL from either 1) www.infura.io  2) www.alchemy.com  3) Self-hosted Geth node
    -ethAcctAddr <ETH_ACCT_ADDR> \ # Only required if you wish to use an existing ETH wallet
    -orchestrator \
    -transcoder \
    -nvidia <NVIDIA_DEVICE_IDs> \ # Only required for transcoding with Nvidia GPUs
    -pricePerUnit <PRICE_PER_UNIT> \ # Your price per pixel in Wei
